...
void manipulating()
{
    ...
    do
    {
        ...
        haystack[strlen(haystack) - 1] = '\0'; // Remove the newline at the end
        if (strcmp(haystack, "q") != 0)
        {
            printf("Type the substring:\n");
            fgets(needle, BUFFER_SIZE, stdin);
            needle[strlen(needle) - 1] = '\0';     // Remove the newline at the end
            occurrence = strstr(haystack, needle); // Search for the substring
            if (occurrence)
                printf("'%s' found at %d position\n", needle, (int)(occurrence - haystack)); // Print the position
            else
                printf("Not found\n");
        }
    } while (strcmp(haystack, "q") != 0); // Continue until 'q' is entered
