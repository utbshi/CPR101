...
void tokenizing()
{
    ...
    do
    {
        ...
        sentences[strlen(sentences) - 1] = '\0'; // Remove the newline at the end
        if ((strcmp(sentences, "q") != 0))
        {
            nextSentence = strtok(sentences, "."); // Tokenize the sentences by '.'
            sentencesCounter = 1;
            while (nextSentence)
            {
                printf("Sentence #%d is '%s'\n", sentencesCounter++, nextSentence); // Print each sentence
                nextSentence = strtok(NULL, ".");                                   // Get the next token
            }
        }
    } while (strcmp(sentences, "q") != 0); // Continue until 'q' is entered
